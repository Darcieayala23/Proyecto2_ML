...
min_samples = 2 * X.shape[1]  # 2 * 11 = 22, para minPts en DBSCAN

neighbors = NearestNeighbors(n_neighbors=min_samples) # NearestNeighbors incluye al propio
                                                        # Por lo tanto, de las 22 columnas devueltas:
                                                        #   columna 0        -> el punto mismo (dist = 0)
                                                        #   columnas 1 a 21   -> los 21 vecinos reales más cercanos
...
plt.grid(True)
plt.tight_layout()
plt.savefig(os.path.join(FIG_DIR, "K-distance_eps_estimado_11D.png"), dpi=150)
plt.show()
...
for eps_test in np.arange(0.50, 7.5, 0.25):
    db = DBSCAN(eps=eps_test, min_samples=22).fit(X)
    labels = db.labels_
...
fig.tight_layout()
plt.savefig(os.path.join(FIG_DIR, "silhouette_vs_eps_11D.png"), dpi=150)
plt.show()

# =====================================================
# Resultados de: Eps elegido: 3.00, min_samples = 22 (2 * n_dimensiones)
# =====================================================

tiempos = []
for _ in range(5):
    t0 = time.time()
    DBSCAN(eps=3.00, min_samples=22).fit(X)
    tiempos.append(time.time() - t0)

t_dbscan_11d = np.mean(tiempos)
print(f"Tiempo promedio DBSCAN (11D, 5 corridas): {t_dbscan_11d:.4f} ± {np.std(tiempos):.4f} segundos")

# Ahora sí, la corrida "real" que usamos para el análisis (labels, crosstabs, etc.)
db_final = DBSCAN(eps=3.00, min_samples=22).fit(X)
...
# Confirmar que el 100% del cluster 1 tiene is_tp=0
print(df_pca[df_pca["cluster_dbscan"] == 1]["is_tp"].value_counts())
...
# ------------------------------------------------------------
# Gráfico de DBSCAN vs. especies reales (proyección PC01-PC02)
# ------------------------------------------------------------

fig, axes = plt.subplots(1, 2, figsize=(14, 6))
 
# Panel izquierdo: coloreado por cluster de DBSCAN
# ------------------------------------------------------------
labels_unicas = sorted(df_pca["cluster_dbscan"].unique())
colores = plt.cm.tab10(np.linspace(0, 1, len(labels_unicas)))
 
for lab, color in zip(labels_unicas, colores):
    mask = df_pca["cluster_dbscan"] == lab
    nombre = "Ruido" if lab == -1 else f"Cluster {lab}"
    marker = "x" if lab == -1 else "o"
    alpha = 0.3 if lab == -1 else 0.7
    axes[0].scatter(df_pca.loc[mask, "PC01"], df_pca.loc[mask, "PC02"],
                     label=nombre, color=color, marker=marker, alpha=alpha, s=25)
 
axes[0].set_xlabel("PC01")
axes[0].set_ylabel("PC02")
axes[0].set_title("Clusters encontrados por DBSCAN")
axes[0].legend()
axes[0].grid(True, alpha=0.3)
 
# Panel derecho: coloreado por species_id real (para comparar)
# ------------------------------------------------------------
especies_unicas = sorted(df_pca["species_id"].unique())
colores_esp = plt.cm.Set2(np.linspace(0, 1, len(especies_unicas)))
 
for esp, color in zip(especies_unicas, colores_esp):
    mask = df_pca["species_id"] == esp
    axes[1].scatter(df_pca.loc[mask, "PC01"], df_pca.loc[mask, "PC02"],
                     label=f"Especie {esp}", color=color, alpha=0.6, s=25)
 
axes[1].set_xlabel("PC01")
axes[1].set_ylabel("PC02")
axes[1].set_title("Distribución real de especies (species_id)")
axes[1].legend()
axes[1].grid(True, alpha=0.3)
 
fig.suptitle("Comparación: clusters DBSCAN vs. especies reales (proyección PC01-PC02)")
fig.tight_layout()
plt.savefig(os.path.join(FIG_DIR, "dbscan_vs_especies_2d_11D.png"), dpi=150)
plt.show()
